...
const SECTION_SCHEMAS={
 ...
 gameAwards:{title:'Game Awards',array:'gameAwards',fields:[['gameId','Select game','gameselect',true,'basic'],['awardType','Award type','awardselect',true,'basic'],['playerId','Select player','playerselect',true,'basic'],['videoUrl','YouTube video URL','url',true,'basic'],['status','Show on website','select',['published','hidden'],'basic'],['customTitle','Optional custom award title','text',false,'advanced'],['order','Order within the game','number',false,'advanced']]}
};
...
async function ghGet(path){const cleanPath=String(path).split('?')[0];const url=`https://api.github.com/repos/${CONFIG.owner}/${CONFIG.repo}/contents/${cleanPath}?ref=${encodeURIComponent(CONFIG.branch)}&_=${Date.now()}`;const r=await fetch(url,{headers:authHeaders(),cache:'no-store'});if(!r.ok)throw new Error(`${r.status}: ${await r.text()}`);return r.json()}
...
function friendlyOption(v){const m={published:'Visible',hidden:'Hidden',placeholder:'Placeholder',latest:'Latest Games',archive:'Archive / older games',core:'Full Matches / Highlights / Slideshows',best:'Best Of',goals:'Goals',saves:'Saves',assists:'Assists',plays:'Plays',shorts:'Shorts',new:'NEW GAME',latest:'LATEST GAME','current-season':'CURRENT SEASON','last-season':'LAST SEASON',player:'Player of the Game',goal:'Goal of the Game',assist:'Assist of the Game',save:'Save of the Game',play:'Play of the Game','':'No label'};return m[v]||v||'Not assigned / N/A'}
function seasonRankAdmin(value){const text=String(value||'');const year=Number((text.match(/\b(19|20)\d{2}\b/)||[])[0])||0;const name=text.toLowerCase();const part=name.includes('winter')?1:name.includes('spring')?2:name.includes('summer')?3:name.includes('fall')||name.includes('autumn')?4:0;return year*10+part;}
...
function inputHtml(field,val){
 ...
 if(type==='gameselect'){
   const games=[...(state.data.games||[])].filter(g=>g&&g.id).sort((a,b)=>seasonRankAdmin(b.season)-seasonRankAdmin(a.season)||(Number(a.gameNumber)||9999)-(Number(b.gameNumber)||9999));
   return `<div class="field full"><label>${label}</label><select name="${key}" ${required}><option value="">Select a game…</option>${games.map(g=>`<option value="${esc(g.id)}" ${String(val)===String(g.id)?'selected':''}>${esc(g.season||'Season')} — Game ${esc(g.gameNumber||'')} — vs ${esc(g.opponent||'Opponent')}</option>`).join('')}</select><div class="help">Season, game number, opponent, and score are filled automatically from the selected game.</div></div>`;
 }
 if(type==='playerselect'){
   const players=[...(state.data.players||[])].filter(p=>p&&p.id&&p.status!=='hidden').sort((a,b)=>(Number(a.order)||9999)-(Number(b.order)||9999));
   return `<div class="field full"><label>${label}</label><select name="${key}" ${required}><option value="">Select a player…</option>${players.map(p=>`<option value="${esc(p.id)}" ${String(val)===String(p.id)?'selected':''}>#${esc(p.number||'—')} — ${esc(p.name||[p.firstName,p.lastName].filter(Boolean).join(' '))} — ${esc(p.position||'Position')}</option>`).join('')}</select><div class="help">Name, jersey number, position, and player photo are filled automatically.</div></div>`;
 }
 if(type==='awardselect'){
   const options=['player','goal','assist','save','play'];
   return `<div class="field full"><label>${label}</label><select name="${key}" ${required}>${options.map(option=>`<option value="${option}" ${String(val||'player')===option?'selected':''}>${esc(friendlyOption(option))}</option>`).join('')}</select><div class="help">The website automatically applies the correct icon and silver, red, green, blue, or purple theme.</div></div>`;
 }
...
function renderDashboard(){
 ...
 if(!fullControl){
   ...
   $('#content').innerHTML=`
    ...
    <div class="quick-management-grid">
     ...
     <a class="quick-management-card" href="game-awards.html"><span class="quick-management-icon">🏆</span><strong>Game Awards</strong><small>Add award videos by selecting a game, award, and player</small><b>${visible('gameAwards')} visible awards</b></a>
 ...
 const groups=[
  ['CONTENT',[
   ...
   ['game-awards.html','🏆','Game Awards','Select a game, award type, player and video link',`${visible('gameAwards')} visible awards`],
...
(() => {
  ...
  function gamePreview(obj, image) {
    return `<div class="visual-card wide-preview-card game-preview-card ${obj.status==='hidden'?'preview-hidden':''}">
      <div class="preview-media-main" style="background-image:url('${previewEsc(image || '../generated/media-card-background.png')}')">${obj.cardLabel?`<span class="preview-game-label label-${previewEsc(obj.cardLabel)}">${previewEsc(friendlyOption(obj.cardLabel))}</span>`:''}</div>
  ...
  function awardPreview(obj) {
    const game=(state.data.games||[]).find(item=>String(item.id)===String(obj.gameId))||{};
    const player=(state.data.players||[]).find(item=>String(item.id)===String(obj.playerId))||{};
    const meta={player:['🏆','PLAYER OF THE GAME','#dfe7ef'],goal:['⚽','GOAL OF THE GAME','#ff3131'],assist:['➤','ASSIST OF THE GAME','#39ff14'],save:['✋','SAVE OF THE GAME','#20bfff'],play:['★','PLAY OF THE GAME','#b44cff']}[obj.awardType]||['★','PLAY OF THE GAME','#b44cff'];
    const photo=previewImage(player.photo||state.data.assets?.playerSilhouette||'images/team/players/player-silhouette.png');
    const name=player.name||[player.firstName,player.lastName].filter(Boolean).join(' ')||'SELECT PLAYER';
    return `<div class="visual-card preview-award-card" style="--preview-accent:${meta[2]}"><div class="preview-award-title"><span>${meta[0]}</span><strong>${previewEsc(obj.customTitle||meta[1])}</strong></div><div class="preview-award-body"><img src="${previewEsc(photo)}" alt=""><div><h3>${previewEsc(name)}</h3><p>#${previewEsc(player.number||'—')} • ${previewEsc(player.position||'POSITION')}</p><small>${previewEsc(game.season||'SEASON')} • GAME ${previewEsc(game.gameNumber||'—')}</small><b>ALLSTAR GALAXY VS ${previewEsc(game.opponent||'OPPONENT')}</b></div></div><span class="preview-visibility">${visibleLabel(obj.status)}</span></div>`;
  }
  ...
  function renderVisualPreview(section, obj, image) {
    ...
    if (section === 'gameAwards') return awardPreview(obj);
  ...
  renderLive = function() {
    ...
    $('#liveUpload').onchange=async e=>{if(!e.target.files[0])return;const r=await fileToPng(e.target.files[0]),path=`images/live/current-livestream-${Date.now().toString(36)}.png`;state.data.live.thumbnail=path;state.pendingFiles=state.pendingFiles.filter(f=>f.path!==path);state.pendingFiles.push({path,base64:r.base64});thumb=URL.createObjectURL(e.target.files[0]);markDirty();draw()};
...
(() => {
  ...
  function renderSelectableManager(section) {
    ...
    $('#previewBtn').onclick=()=>{sessionStorage.setItem('asgPreviewMasterContent',JSON.stringify(state.data)); const pages={players:'team.html',games:'media.html',gameAwards:'media.html',seasons:'media.html',playlists:'media.html'}; window.open('../'+pages[section]+'?adminPreview=1','_blank');};
