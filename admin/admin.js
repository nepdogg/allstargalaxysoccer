...
/* V196 Game Awards Admin — shared renderer, correct preview route and dedicated click behavior */
(() => {
  const baseRenderPage=renderPage;
  const awardTypes=['Player of the Game','Goal of the Game','Save of the Game','Assist of the Game','Play of the Game'];
  ...
  function openAwardForm(index){
    const arr=awardArray(),item=index>=0?JSON.parse(JSON.stringify(arr[index])):{id:nextId(arr,'game-award'),awardType:'Player of the Game',status:'published',order:arr.length+1,gameId:state.data.games?.[0]?.id||'',playerId:state.data.players?.[0]?.id||'',videoUrl:''};state.editIndex=index;$('#modalTitle').textContent=(index>=0?'Edit':'Add')+' Game Award';
    const gameOptions=(state.data.games||[]).map(g=>`<option value="${esc(g.id)}" ${item.gameId===g.id?'selected':''}>${esc(gameLabel(g))}</option>`).join('');const playerOptions=(state.data.players||[]).filter(p=>p.status!=='hidden').map(p=>`<option value="${esc(p.id)}" ${item.playerId===p.id?'selected':''}>${esc(playerLabel(p))}</option>`).join('');
    $('#editForm').innerHTML=`<div class="form-section"><h4>Basic Settings</h4><div class="form-grid"><div class="field full"><label>Select Game</label><select name="gameId" id="awardGameSelect" required>${gameOptions}</select><div class="help">Season, game number, opponent and score come from the selected game.</div></div><div class="field full"><label>Award Type</label><select name="awardType" id="awardTypeSelect">${awardTypes.map(x=>`<option ${item.awardType===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="field full"><label>Select Player</label><select name="playerId" id="awardPlayerSelect" required>${playerOptions}</select><div class="help">Name, jersey number, position and player photo come from the selected player.</div></div><div class="field"><label>YouTube Award Video URL</label><input name="videoUrl" type="url" value="${esc(item.videoUrl||item.url||'')}" placeholder="https://youtu.be/..."></div><div class="field"><label>Show on Website</label><select name="status"><option value="published" ${item.status!=='hidden'?'selected':''}>Visible</option><option value="hidden" ${item.status==='hidden'?'selected':''}>Hidden</option></select></div></div></div><details class="advanced-box"><summary>Advanced Settings</summary><div class="form-grid"><div class="field"><label>Carousel Order</label><input name="order" type="number" value="${esc(item.order||arr.length+1)}"></div></div></details><div class="game-award-form-preview"><span class="v2-pill">LIVE PREVIEW</span><div id="awardFormPreview"></div></div><div class="admin-actions form-actions"><button class="btn primary" type="submit">Save Form</button><button class="btn" type="button" id="cancelForm">Cancel</button></div>`;
    const form=$('#editForm');const draft=()=>({...item,gameId:$('#awardGameSelect').value,awardType:$('#awardTypeSelect').value,playerId:$('#awardPlayerSelect').value,videoUrl:form.elements.videoUrl.value,status:form.elements.status.value,order:Number(form.elements.order.value)||arr.length+1});const draw=()=>$('#awardFormPreview').innerHTML=awardPreview(draft());form.querySelectorAll('select,input').forEach(el=>el.addEventListener('input',draw));draw();
    form.onsubmit=e=>{e.preventDefault();const obj=draft();const game=(state.data.games||[]).find(g=>g.id===obj.gameId)||{};const player=(state.data.players||[]).find(p=>p.id===obj.playerId)||{};obj.gameSnapshot={season:game.season,gameNumber:game.gameNumber,opponent:game.opponent,result:game.result};obj.playerSnapshot={name:player.name,firstName:player.firstName,lastName:player.lastName,number:player.number,position:player.position,photo:player.photo};if(index>=0)arr[index]=obj;else arr.unshift(obj);markDirty();closeModal();state.awardSelected=index>=0?index:0;renderAwardAdmin()};$('#cancelForm').onclick=closeModal;$('#editModal').classList.add('open');
  }
  renderPage=function(){const page=document.body.dataset.page||'dashboard';if(page==='gameAwards'){$$('.admin-nav a').forEach(a=>a.classList.toggle('active',a.dataset.page===page));renderAwardAdmin()}else baseRenderPage()};
})();
