...
(() => {
  ...
  function playerPreview(obj, image) {
    const parts=String(obj.name||'PLAYER NAME').trim().split(/\s+/); const first=parts.shift()||'PLAYER'; const last=parts.join(' ')||first;
    return `<div class="visual-card player-preview-card public-style-player-preview ${obj.status==='hidden'?'preview-hidden':''}">
      <div class="preview-ultimate-card">
        <div class="preview-card-top"><div><b>${previewEsc(obj.number||'00')}</b><small>${previewEsc(obj.position||'PLAYER')}</small></div><img src="../images/logos/logo.png" alt=""></div>
        <img class="preview-ultimate-photo" src="${previewEsc(image || '../images/team/players/player-silhouette.png')}" alt="">
        <div class="preview-ultimate-name"><small>${previewEsc(first)}</small><strong>${previewEsc(last)}</strong><em>★ ALLSTAR GALAXY ★</em></div>
      </div>
      ${obj.profileMode==='advanced'?`<div class="preview-profile-badge">ADVANCED • DOUBLE CARD</div>`:''}
