(() => {
  ...
  function scheduleMarkup(data){
    ...
    if((display.mode||'images')==='images'){
      const schedule=display.scheduleVisible!==false && scheduleImage ? `<button class="generated-schedule-image-card" type="button" data-schedule-image="${esc(scheduleImage)}" data-schedule-title="Match Schedule"><img src="${esc(scheduleImage)}" alt="Current match schedule"><span>VIEW FULL SCHEDULE</span></button>` : '';
      const standings=display.standingsVisible!==false && standingsImage ? `<button class="generated-schedule-image-card" type="button" data-schedule-image="${esc(standingsImage)}" data-schedule-title="League Standings"><img src="${esc(standingsImage)}" alt="Current league standings"><span>VIEW FULL STANDINGS</span></button>` : '';
      return `<div class="generated-schedule-images">${schedule}${standings}</div>`;
    }
